...
public class CubeStatusController {
    ...
    /**
     * checks if Cube is already in database, create a new entry if not and sets actual {@link CubeStatus}
    ...
    public Cube updateCube(StatusMessage message) {
        ...
        LOGGER.info("cube {} was updated and set status to {}", updatedCube.getId(),
                this.cubeStatus.get(updatedCube.getMacAddress()).getStatus());
    ...
    /**
     * @return health status of all online cubes i.e. with status {@link CubeStatus.READY, CubeStatus.LIVE or
     *         CubeStatus.IN_CONFIGURATION}
     */
    ...
    public CubeStatus getStatus(String macAddress) {
        if (this.cubeStatus.get(macAddress) != null) return this.cubeStatus.get(macAddress).getStatus();
        else {
            return null;
        }
    }
    ...
    /**
     * scheduled task which checks in a fixed interval which cubes are online; if online cubes are not reporting in the
     * given reporting interval (set by admin) their status is set to OFFLINE and their healthStatus is removed
     */
    ...
    public void updateHealthStatus() {
        ...
        for (Map.Entry<String, HealthStatus> m : healthStatus.entrySet()) {
            if (m.getValue().getTimestamp().isBefore(
                ...
                setOffline(m.getKey());
                LOGGER.info("[{}] cube with mac {} changed to status OFFLINE", LocalDateTime.now().format(myFormat),
                        m.getKey());
            }
            else {
                if (m.getValue().getBatteryLevel() < cubeService
                        .queryThreshold(ThresholdType.BATTERY_LEVEL_THRESHOLD)) {
                    LOGGER.info("[{}] cube with mac {} has low battery (level at {})",
                            LocalDateTime.now().format(myFormat), m.getKey(), m.getValue().getBatteryLevel());
                }
                if (m.getValue().getRssi() < cubeService.queryThreshold(ThresholdType.RSSI_THRESHOLD)) {
                    LOGGER.info("[{}] cube with mac {} reported rssi level at {}", LocalDateTime.now().format(myFormat),
                            m.getKey(), m.getValue().getRssi());
                }
            }
        }
    }
